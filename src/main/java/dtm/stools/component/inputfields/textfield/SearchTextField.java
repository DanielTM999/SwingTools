...
public class SearchTextField<T> extends MaskedTextField{
    ...
    private final AtomicBoolean ignoreCurrentSugestions = new AtomicBoolean(false);
    private final AtomicBoolean selectingFromList = new AtomicBoolean(false);
    private final AtomicBoolean caseSensitive = new AtomicBoolean(false);
    private final AtomicBoolean autoCloseOnFocusLost = new AtomicBoolean(true);
    ...
    public SearchTextField(String mask, char placeholder){
        this(mask, placeholder, 0);
    }

    public SearchTextField(String mask, char placeholder, int columns){
        super(mask, placeholder, columns);
        ...
        this.executorRef = new AtomicReference<>(Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r);
            t.setName("SearchTextField-Worker-" + System.identityHashCode(this));
            t.setDaemon(true);
            return t;
        }));
        ...
        addEventListener(EventType.INPUT, e -> {
            if(ignoreCurrentSugestions.getAndSet(false))return;
            if (selectingFromList.get()) return;
    ...
    public void setAutoCloseOnFocusLost(boolean autoClose) {
        this.autoCloseOnFocusLost.set(autoClose);
    }

    public void ignoreCurrentSugestions(){
        this.ignoreCurrentSugestions.set(true);
    }
    ...
    private List<T> search(String text) {
        ...
        String searchText = caseSensitive.get() ? text : text.toLowerCase();
        ...
        synchronized (dataSources) {
            for (T item : dataSources) {
                ...
                for (Function<T, String> fn : searchOptions) {
                    ...
                    if (value != null) {
                        String compareValue = caseSensitive.get() ? value : value.toLowerCase();
    ...
    private List<T> getAllResults() {
        List<T> results;
        synchronized (dataSources) {
            results = new ArrayList<>(dataSources);
        }
        if (sortComparator != null) {
            results.sort(sortComparator);
        }
    ...
    private void showSuggestions(List<T> results) {
        if (results == null || results.isEmpty() || !this.isShowing()) {
            suggestionPopup.setVisible(false);
            return;
        }
    ...
    private void selectSuggestion(T selected) {
        ...
        try {
            ...
            dispachEvent(EventType.SELECT, selected);
        } finally {
            SwingUtilities.invokeLater(() -> selectingFromList.set(false));
        }
    }
    ...
    private void addFocusListenerAutoClose() {
        addFocusListener(new FocusAdapter() {
            ...
            public void focusLost(FocusEvent e) {
                if (autoCloseOnFocusLost.get() && !e.isTemporary()) {
        ...
        while (true) {
            ...
            if (executor == null || executor.isShutdown() || executor.isTerminated()) {
                ExecutorService newExecutor = Executors.newSingleThreadExecutor(r -> {
                    Thread t = new Thread(r);
                    t.setName("Search-Field-Worker-" + System.identityHashCode(this));
                    t.setDaemon(true);
                    return t;
                });
